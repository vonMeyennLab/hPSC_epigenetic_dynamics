...
subtitle: "Profiles from ATAC-seq regions open in the Naïve state"
...
# Loading results from MOFA2 factor 1 (ATAC-seq Naive)
```{r}
...
# Selecting ATAC-Seq naive significant regions
regions <- userSets_signif$`ATAC-seq.Naive (7695)` %>% resize(1, fix = "center")
...
map2(names(.),
  ~ EnrichedHeatmap(.x, 
                    ...
                    use_raster           = T,
                    raster_by_magick     = T,
                    row_split            = factor(rep(c("HNES1 d0", "cR-H9 d0", "HNES1 d10", "cR-H9 d10", "cR-H9 d20X", "cR-H9 d20E", "H9 Primed"), each = 19397), levels = c("HNES1 d0", "cR-H9 d0", "HNES1 d10", "cR-H9 d10", "cR-H9 d20X", "cR-H9 d20E", "H9 Primed")),
...
Heatmap(matrix               = list(distance_df,distance_df,distance_df,distance_df,distance_df,distance_df,distance_df) %>% purrr::reduce(rbind), 
        name                 = "Condition", 
...
# Normalized matrix of CpG islands
CGI_normalized_matriz_list <-
  normalizeToMatrix(
    signal = genome_annotation$`CpG islands` %>%
      as.data.frame() %>%
      mutate(score = 1) %>%
      makeGRangesFromDataFrame(keep.extra.columns = T) %>%
      .[seqnames(.) %in% 1:22],
...
EnrichedHeatmap(list(CGI_normalized_matriz_list,CGI_normalized_matriz_list,CGI_normalized_matriz_list,CGI_normalized_matriz_list,CGI_normalized_matriz_list,CGI_normalized_matriz_list,CGI_normalized_matriz_list) %>% purrr::reduce(rbind),
                name           = "CpG islands",
                column_title   = "CpG islands",
                cluster_rows   = F,
                row_split      = factor(rep(c("HNES1 d0", "cR-H9 d0", "HNES1 d10", "cR-H9 d10", "cR-H9 d20X", "cR-H9 d20E", "H9 Primed"), each = 19397), levels = c("HNES1 d0", "cR-H9 d0", "HNES1 d10", "cR-H9 d10", "cR-H9 d20X", "cR-H9 d20E", "H9 Primed")),
...
Heatmap(matrix = factor(rep(c("HNES1 d0", "cR-H9 d0", "HNES1 d10", "cR-H9 d10", "cR-H9 d20X", "cR-H9 d20E", "H9 Primed"), each = 19397), levels = c("HNES1 d0", "cR-H9 d0", "HNES1 d10", "cR-H9 d10", "cR-H9 d20X", "cR-H9 d20E", "H9 Primed")), 
...
png(filename = "output/figures/03_condition_comparison/ATACseq_naive_signif_ht.png", width = 12, height = 10, res = 300, units = "in")
ht_list
dev.off()

svg(filename = "output/figures/03_condition_comparison/ATACseq_naive_signif_profile.svg", width = 15, height = 1.75)
