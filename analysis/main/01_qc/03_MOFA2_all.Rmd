...
# RNA-seq read counts, calculate the average between replicates, and log-transform
```{r}
...
# Log-transform
RNAseq_normalized_counts <- log2(RNAseq_normalized_counts + 1)
...
RNAseq_normalized_counts <- RNAseq_normalized_counts %>% .[rownames(.) %in% genes_id_in_autosomes, ] %>% .[rownames(.) %in% DGE_gene_ids, ]

```

# PBAT read counts and calculate the average between replicates
```{r}
...
colnames_order <- chipseq_atacseq_transformed_counts$`ATAC-Seq` %>% colnames %>% .[!str_detect(., "d3")]

# Creating the MOFA object
MOFAobject <- create_mofa(
                          list(
                            "RNA-seq"     = RNAseq_normalized_counts[, colnames_order] %>% as.matrix(),
...
# loading the model
model <- load_model("tmp/MOFA2.hdf5")
...
svg(filename = "output/figures/01_qc/MOFA2_plot_1.svg", width = 3.25, height = 3)
#pdf(file = "output/figures/01_qc/MOFA2_plot_1.pdf"    , width = 3.25, height = 3)
tsne_plot
dev.off()


svg(filename = "output/figures/01_qc/MOFA2_plot_2.svg", width = 10, height = 5)
#pdf(file = "output/figures/01_qc/MOFA2_plot_2.pdf"    , width = 10, height = 5)
